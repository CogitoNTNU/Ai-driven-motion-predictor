...
```bash
docker compose -f ../../docker-compose.yaml up -d  # PostgreSQL on port 5432, pgAdmin on port 5050
```
...
1. **Storage** — A PostgreSQL database (via Docker) stores OHLCV market data. SQL query patterns are in `queries/example_queries.sql` (uses DuckDB syntax).
1. **Modeling** — Jupyter notebooks in `src/Practice/` and `src/tasks.ipynb` contain exploratory analysis and model development (scikit-learn).
1. **Module** — `src/Kaare/` is the primary module directory for this branch's work.
